...
/// Synchronize the thread-local `GLOBAL_SETTINGS` with the current `RegionCtx` settings.
///
/// Why this exists:
/// - The Einsum chip’s analysis/lowering logic (including probabilistic/Freivalds strategy selection)
///   is currently driven off `graph::config::GLOBAL_SETTINGS`.
/// - During analysis/dummy layout (and in some nested lowering paths like Conv→Einsum), the global
///   settings may be unset or stale unless we explicitly sync them from the current region.
/// - This function ensures `PolyOp::Einsum` and `PolyOp::Conv` trigger the correct strategy when
///   `RegionSettings` enables probabilistic execution.
///
/// Note:
/// - We intentionally keep this local to ops/poly.rs because Einsum/Conv are the entry points that
///   must guarantee the correct analysis behavior in both real and dummy regions.
fn sync_global_settings_from_region<
...
>(
    ...
    GLOBAL_SETTINGS.with(|gs| {
        ...
        // This avoids requiring `Clone` and ensures we can initialize the global settings
        // during dummy/analysis passes where it might otherwise be `None`.
        let mut gs_ref = gs.borrow_mut();
        let mut settings = gs_ref.take().unwrap_or_default();

        // Sync execution mode.
        settings.execution_mode = match region.execution_mode() {
            CmdExecutionMode::Exact => crate::ExecutionMode::Exact,
            CmdExecutionMode::Probabilistic => crate::ExecutionMode::Probabilistic,
        };

        // Sync probabilistic k (Freivalds repetitions).
        settings.probabilistic_settings.k_repetitions = region.prob_k();

        // Sync op policy list (MatMul/Gemm/Conv).
        // RegionSettings carries a Vec<String> policy; convert into ProbOps.
        // Unknown strings are ignored (forward-compat / permissive parsing).
        let mut prob_ops = ProbOps::default();
        for op in region.prob_ops().iter().map(|s| s.as_str()) {
            match op {
                "MatMul" | "matmul" | "MATMUL" => {
                    let _ = prob_ops.insert(ProbOp::MatMul);
                }
                "Gemm" | "gemm" | "GEMM" => {
                    let _ = prob_ops.insert(ProbOp::Gemm);
                }
                "Conv" | "conv" | "CONV" => {
                    let _ = prob_ops.insert(ProbOp::Conv);
                }
                _ => {}
            }
        }
        settings.prob_ops = prob_ops;

        // Restore updated settings into the thread-local.
        *gs_ref = Some(settings);
    });
}
    ...
    > Op<F> for PolyOp
{
    ...
    fn layout(
        ...
        Ok(Some(match self {
            ...
            // Step 6: Ensure Einsum sees RegionSettings-driven probabilistic config.
            PolyOp::Einsum { equation } => {
                sync_global_settings_from_region(region);
            ...
            // Step 6: Conv lowers into Einsum; ensure the Einsum analysis sees region settings.
            PolyOp::Conv {
                padding,
                stride,
                group,
