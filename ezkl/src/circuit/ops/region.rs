use crate::{
    ...
    commands::{ExecutionMode, ProbSeedMode},
...
const DEFAULT_PROB_K: u32 = 40;
const DEFAULT_PROB_OPS: [&str; 3] = ["MatMul", "Gemm", "Conv"];
...
pub struct RegionSettings {
    ...
    /// which ops should use probabilistic verification (e.g., MatMul/Gemm/Conv)
    pub prob_ops: Vec<String>,
    /// number of Freivalds repetitions
    pub prob_k: u32,
    /// seed mode for probabilistic challenges
...
impl<'a, F: PrimeField + TensorType + PartialOrd + std::hash::Hash> RegionCtx<'a, F> {
    ...
    /// execution mode for this region
    pub fn execution_mode(&self) -> ExecutionMode {
        self.settings.execution_mode
    }

    /// probabilistic ops list for this region
    pub fn prob_ops(&self) -> &[String] {
        &self.settings.prob_ops
    }

    /// number of Freivalds repetitions configured for this region
    pub fn prob_k(&self) -> u32 {
        self.settings.prob_k
    }
