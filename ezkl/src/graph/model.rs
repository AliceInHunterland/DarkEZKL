...
/// RAII helper to safely set/reset the thread-local GLOBAL_SETTINGS within this thread.
///
/// Probabilistic execution uses GLOBAL_SETTINGS inside the einsum chip to decide whether
/// to apply Freivalds-style checks. During configuration and dummy/prover layouts we must
/// ensure this is set consistently so sizing and constraints match the intended execution mode.
#[derive(Debug)]
struct GlobalSettingsGuard {
    prev: Option<GraphSettings>,
}

impl Drop for GlobalSettingsGuard {
    fn drop(&mut self) {
        GLOBAL_SETTINGS.with(|gs| {
            *gs.borrow_mut() = self.prev.take();
        });
    }
}

fn set_global_settings(settings: Option<GraphSettings>) -> GlobalSettingsGuard {
    let prev = GLOBAL_SETTINGS.with(|gs| gs.borrow().clone());
    GLOBAL_SETTINGS.with(|gs| {
        *gs.borrow_mut() = settings;
    });
    GlobalSettingsGuard { prev }
}

/// Build a minimal GraphSettings snapshot from RunArgs for use in dummy/prover layouts.
///
/// We only need the "execution contract" fields to be correct here:
/// - execution_mode
/// - probabilistic_settings (k_repetitions, seed_mode)
/// - prob_ops
fn settings_snapshot_from_run_args(run_args: &RunArgs) -> GraphSettings {
    let mut gs = GraphSettings::default();

    gs.run_args = run_args.clone();

    // Mirror execution contract fields at top-level.
    gs.execution_mode = run_args.execution_mode;
    gs.prob_k = run_args.prob_k as u32;
    gs.prob_ops = run_args.prob_ops.clone();
    gs.probabilistic_settings = ProbabilisticSettings {
    ...
    gs.run_args.execution_mode = gs.execution_mode;
...
///
/// For classic 0/1 Freivalds checks, if a single matrix-product check is repeated `k` times,
/// the per-check soundness error is ≤ 2^-k. If there are `N` independent checks, a simple union
/// bound gives:
///
///     p_fail_total <= N * 2^-k
///
/// We log this during the "compilation / setup" stage (gen_params) so users can audit and size
/// a security budget from their settings / model.
///
/// IMPORTANT:
/// - This is an *upper bound* intended for visibility, not a formal proof.
/// - Today we conservatively estimate `N` using the number of einsum equations recorded by the
///   dummy layout pass, since einsum is where Freivalds-style verification is currently applied.
/// - Future steps can refine `N` once we have explicit per-op probabilistic lowering metadata.
fn log_probabilistic_soundness_budget(run_args: &RunArgs, dummy_res: &DummyPassRes) {
    if run_args.execution_mode != ExecutionMode::Probabilistic {
        return;
    }

    if run_args.disable_freivalds {
        warn!(
            "execution_mode=probabilistic but disable_freivalds=true; no Freivalds-style checks \
             will be enforced (soundness budget is not applicable / is effectively unsafe)."
        );
        return;
    }

    // Estimate number of probabilistic checks.
    // Current proxy: number of einsum equations used in the circuit.
    let num_checks = dummy_res.einsum_params.equations.len();

    let k = run_args.prob_k as u32;
    ...
    // Union bound: p_fail_total <= N * 2^-k.
    // Compute a conservative "bits of security" view:
    //   N * 2^-k <= 2^{ceil(log2 N) - k} = 2^-((k) - ceil(log2 N))
    let log2_n = (num_checks as f64).log2();
    let ceil_log2_n = log2_n.ceil() as u32;
    let union_bound_bits = k.saturating_sub(ceil_log2_n);

    // Provide a floating-point approximation when representable.
    // (For very small values this may underflow to 0.0, but the log form remains informative.)
    let approx_total_error = (num_checks as f64) * 2f64.powi(-(k as i32));

    info!(
        "probabilistic soundness budget (union bound): \
         checks(N)={}, k_repetitions={}, per_check<=2^-{}, \
         total_error<=N*2^-k≈{:.3e} (<=2^-{}). \
         prob_ops={}, seed_mode={}.",
        num_checks,
        k,
        k,
...
impl Model {
    ...
    pub fn configure(
        ...
        // Step 6: ensure GLOBAL_SETTINGS reflects the intended execution contract during circuit
        // configuration, so einsum configuration is consistent with probabilistic settings.
        let _gs_guard = set_global_settings(Some(settings.clone()));
    ...
    pub fn layout(
        ...
        // Step 6: set GLOBAL_SETTINGS during layout so probabilistic einsum paths
        // (Freivalds checks) are enabled/disabled consistently.
        let _gs_guard = set_global_settings(Some(settings_snapshot_from_run_args(run_args)));
    ...
    pub fn dummy_layout(
        ...
        // Step 6: set GLOBAL_SETTINGS during dummy layout so circuit sizing reflects
        // probabilistic vs exact execution consistently.
        let _gs_guard = set_global_settings(Some(settings_snapshot_from_run_args(run_args)));
