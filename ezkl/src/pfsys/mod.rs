...
fn ceil_32(n: usize) -> usize {
    (n + 31) / 32 * 32
}

fn pad_right_to_32(data: &[u8]) -> Vec<u8> {
    let mut out = data.to_vec();
    let target = ceil_32(out.len());
...
///
/// For `vka`:
/// - Pass `None` if verifying key is embedded in `Halo2Verifier` (encodes `verifyProof(bytes,uint256[])`)
/// - Pass `Some(vka)` if verifying key is separated and already registered (encodes `verifyProof(bytes,uint256[],bytes32[])`)
///
/// NOTE: This must follow Solidity ABI encoding rules for dynamic types:
/// - `bytes` payload must be padded to 32-byte boundary
/// - offsets must account for this padding
pub fn encode_calldata(vka: Option<&[[u8; 32]]>, proof: &[u8], instances: &[bn256::Fr]) -> Vec<u8> {
    ...
    //   offset(proof) (32)
    //   offset(instances) (32)
    //   offset(vka) (32)            [only if vka is Some]
    // tail:
    //   proof_len (32) + proof_bytes (padded)
    //   instances_len (32) + instances (32 each)
    //   vka_len (32) + vka_words (32 each)  [only if vka is Some]
    let has_vka = vka.is_some();
    let fn_sig = if has_vka {
        FN_SIG_VERIFY_PROOF_WITH_VKA
    } else {
        FN_SIG_VERIFY_PROOF
    };

    let head_size = if has_vka { 0x60 } else { 0x40 };

    let proof_padded = pad_right_to_32(proof);
    let proof_tail_size = 0x20 + proof_padded.len();

    let num_instances = instances.len();
    let instances_tail_size = 0x20 + (num_instances * 0x20);

    let (num_vka_words, vka_tail_size) = if let Some(vka) = vka {
        (vka.len(), 0x20 + (vka.len() * 0x20))
    } else {
        (0usize, 0usize)
    };

    let proof_offset = head_size;
    let instances_offset = proof_offset + proof_tail_size;
    let vka_offset = instances_offset + instances_tail_size;

    let mut out = Vec::with_capacity(
        4 + head_size + proof_tail_size + instances_tail_size + vka_tail_size + 64,
    );

    // selector
    out.extend_from_slice(&fn_sig);

    // head offsets
    out.extend_from_slice(&to_be_bytes_32(proof_offset));
    out.extend_from_slice(&to_be_bytes_32(instances_offset));
    if has_vka {
        out.extend_from_slice(&to_be_bytes_32(vka_offset));
    }

    // proof tail
    out.extend_from_slice(&to_be_bytes_32(proof.len()));
    out.extend_from_slice(&proof_padded);

    // instances tail
    out.extend_from_slice(&to_be_bytes_32(num_instances));
    for fr in instances {
        out.extend_from_slice(&fr_to_bytes32(fr));
    }

    // vka tail
    if let Some(vka) = vka {
        out.extend_from_slice(&to_be_bytes_32(num_vka_words));
        for word in vka {
...
where
    // NOTE: use the `halo2curves` crate's `Repr` directly.
    // Some pinned halo2_proofs revisions do not re-export `halo2curves` under `halo2_proofs::halo2curves`.
    F: PrimeField<Repr = halo2curves::serde::Repr<32>>,
{
...
#[cfg(feature = "python-bindings")]
///
pub fn g1_to_pydict(g1_dict: &pyo3::Bound<'_, pyo3::types::PyDict>, g1: &G1) {
